...
import { createI18nContext, persistLanguagePreference, getPersistedLanguagePreference } from './i18n/index.js';
import { persistTripSetupPreference as persistTripSetupCookie, readPersistedTripSetupPreference } from './state/persistence.js';
import { projectBox, rotatePoint3d, createProjector, createBoxVertices, clamp } from './visualization/geometry.js';
...
const { t, localizeEntity } = createI18nContext(() => state.language);
...
function persistTripSetupPreference() {
  ...
  persistTripSetupCookie(payload);
}

function applyPersistedTripSetupPreference() {
  const persisted = readPersistedTripSetupPreference();
  if (!persisted) return;
  try {
    ...
    // no-op; cookie parse handled in persistence module
  }
}
