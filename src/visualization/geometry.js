...
export function rotatePoint3d(point, center, angles) {
  ...
  return { x: rotatedX, y: rotatedY * cosPitch - centeredZ * sinPitch, depth: rotatedY * sinPitch + centeredZ * cosPitch };
}
...
export function createProjector(zone, placements, canvasWidth, canvasHeight, padding, extraPoints = [], angles) {
  ...
  return (point) => {
    ...
    return { x: canvasWidth / 2 + output.x * scale, y: canvasHeight / 2 + output.y * scale, depth: output.depth };
  };
}
